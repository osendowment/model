...
`src.sources.git.clone`. Sparse-checkout is fine — semgrep only needs the source
files, not git history.

Output: data/sources/git/semgrep.csv (long format, shared schema)
...
Usage:
    ...
    uv run python -m src.sources.github.fetch_semgrep --limit 10 --rulepack p/security-audit
    uv run python -m src.sources.github.fetch_semgrep --limit 0   # all eligible (don't!)
"""
...
from src.sources.git.long_format import upsert_snapshot as _upsert_snapshot
...
def rulepack_prefix(rulepack: str) -> str:
    ...
    Mirrors `src.sources.git.migrations.migrate_semgrep.rulepack_prefix` so historical
...
def _load_target_year_shas() -> dict[str, tuple[str, str]]:
    ...
    Mirrors `src.risk.build_complexity._load_target_year` /
    `src.sources.github.fetch_advanced_complexity._load_target_year_shas`. Repos not
