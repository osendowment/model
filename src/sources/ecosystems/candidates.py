        ...
        async with aiohttp.ClientSession(
            ...
            with progress:
                ...
                async def _run(p: str) -> None:
                    ...
                    all_rows[(eco, p)] = await _ensure_row(session, eco, p, ttl_days, sem, counters)
...
def main() -> None:
    ...
    console.print(f"[bold]ecosyste.ms candidate fetch[/bold]  [dim]scope={args.scope} | "
                  f"{_now_iso()} | ttl={args.ttl}d | conc={args.concurrency}[/dim]")
    ...
    for eco in ecosystems:
        c = fetch_eco_sync(eco, args.ttl, args.concurrency, all_rows, flush, args.limit, args.scope)
