...
Data sources:
  DB dump : https://static.crates.io/db-dump.tar.gz
            Provides crate name mapping (crates.csv, versions.csv, dependencies.csv).
            Downloaded to .cache/crates/, extracted to data/crates/, then deleted.
...
DUMP_PATH = f"{CACHE}/db-dump.tar.gz"
...
missing_extracted = [t for t in EXTRACT_TARGETS if not os.path.exists(f"{PATH}/{t}")]

if not missing_extracted:
    print("crates.csv + versions.csv + dependencies.csv already extracted — skipping download")
...
# ── Step 2: Extract crates.csv + versions.csv + dependencies.csv from dump ────

if missing_extracted:
    print(f"\nExtracting {missing_extracted} from dump ...")
    with tarfile.open(DUMP_PATH, "r:gz") as tar:
        for member in tar.getmembers():
            if "/data/" not in member.name:
                continue
            fname = member.name.split("/data/")[-1]
            if fname in missing_extracted:
                member.name = fname
                tar.extract(member, PATH)
                print(f"  Extracted {fname} → {PATH}/{fname}")
    print(f"Deleting dump: {DUMP_PATH}")
    os.remove(DUMP_PATH)
else:
    print("crates.csv + versions.csv + dependencies.csv already extracted — skipping")
