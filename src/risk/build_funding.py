...
Reads (all under data/sources/):
    github/sponsors.csv        — github_sponsors  (src.sources.github.fetch_sponsors)
    github/funding-yml.csv     — has_funding_yml / funding_yml_platforms /
                                 per-platform handles  (src.sources.github.fetch_funding_yml)
    floss-fund/funding-json.csv — FLOSS Fund directory export; has_funding_json
                                 + channel_platforms  (src.sources.floss_fund.funding_json)
    opencollective/budgets.csv — annual gross raised per OC slug
                                 (src.sources.opencollective.fetch_budgets)
