...
`build_<dim>.py` script) and joins them on `repo` into one flat table:
    ...
    data/risk/concentration.csv     ← src.risk.build_concentration
    data/risk/complexity.csv        ← src.risk.build_complexity
    data/risk/security.csv          ← src.risk.build_security
    data/risk/funding.csv           ← src.risk.build_funding
    data/risk/visibility.csv        ← src.risk.build_visibility
    data/risk/workload.csv          ← src.risk.build_workload
...
Usage:
    ...
    uv run python -m src.risk.aggregate_risk --random 5      # test on 5 repos
    uv run python -m src.risk.aggregate_risk --random 50
...
def aggregate(sample: set[str] | None = None) -> tuple[list[str], list[dict[str, str]]]:
    ...
    if missing_files:
        raise SystemExit(
            ...
            + "\n  - ".join(missing_files)
            + "\n\nRun the corresponding `src.risk.build_<dim>` first."
        )
