...
Steps flagged `net=True` do network I/O and receive --refresh when it is set.
Every fetcher declares a TTL in settings.json, so a warm run is already
zero-network without a mode flag — the TTL is the cache policy, and --refresh
is the only override. Steps flagged `pipeline=True` are sub-orchestrators; they
also receive --refresh so they can propagate it to their own net steps.
...
class Step:
    ...
    pipeline: bool = False  # step is itself an orchestrator — receives --refresh
    net: bool = False       # step does network I/O — receives --refresh
...
def select_steps(steps: list[Step], from_step: str | None,
    ...
    EVERY selected step runs. There is no mode that skips fetchers: each one
    declares a TTL in settings.json and no-ops when its cache is warm, so a
    repeat run is already zero-network. Use --refresh to force a refetch.

    This deliberately replaces the old --offline, which dropped every
    `fetch=True and not net=True` step from the run. Those steps did not fall
    back to their caches — they simply did not execute, so their outputs went
    stale or empty while the run still reported success. That is how a value
    rebuild plus an offline eligibility run silently reduced 900 registry
    licences to blanks with all 83 health checks still green.

    Raises KeyError if `from_step` / `only` names an unknown step.
    """
...
def build_parser(description: str) -> argparse.ArgumentParser:
    ...
    p.add_argument("--refresh", action="store_true",
                   help="Force refetch in net steps, ignoring their TTL caches")
