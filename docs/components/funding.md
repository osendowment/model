...
- A scraped FOSS-foundation host (`funding/host-by-repo.csv`) defaults to
  ...
  (host `fsf/gnu` — every GNU package is FSF-stewarded), the **FSF** itself, and
  **Software Freedom Conservancy** members (incl. Sourceware-hosted libs like
  `libffi`) repos drop from 100 → 50. `match_repos` joins each roster to a repo
  by exact `owner/name` slug, a curated GNU/foundation org-prefix
  (`autotools-mirror/*`, `gcc-mirror/*`, `coreutils/*` → GNU), or apex/suffix
  domain (`*.gnu.org` → GNU, `sfconservancy.org` → SFC); reference-only
  subdomains (`peps.python.org`, docs pages) are excluded so a repo that merely
  links to a foundation is not miscredited.
