...
- [ ] **Step 3:** Create `src/pipeline/value/verify_git_urls.py` containing: the module docstring, the imports it needs (`argparse`, `csv`, `re`, `subprocess`, `concurrent.futures`, `pathlib.Path`, `rich`, plus `DATA_DIR`), every verifier function/constant moved out of `unify_value_data.py`, and a `main()` that: reads `data/value/value.csv` into a list of dicts, calls `verify_urls_in_aggregates(rows)`, prints `_print_git_validity_table`, and writes the rows back to `data/value/value.csv` (reuse `unify_value_data.write_value_data` via import, or an inline `csv.DictWriter` with the same `FIELDS`). Guard with `if __name__ == "__main__": main()`.
