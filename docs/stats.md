...
**Risk dimensions cover the 916 top repos** — the valid class-A set
(`risk_input.value_classes = ["A"]`), **archived included** (they enter risk
scope like every other stage; archival surfaces in eligibility as
`active=False`); each funnel below starts from those 916. `risk.csv` ranks all
916 on the four scored dimensions; the funding signals and the intent/nonprofit
flags moved to the [Eligibility](#eligibility) stage. Methodology:
[risk.md](risk.md) + component docs.

Two archived repos (`bincode-org/bincode`, `isaacs/inflight-deprecated-do-not-use`)
have empty/stub snapshots — scc measures 0 lines of code — so complexity and
workload (both need source) are legitimately blank for them; overall
`risk_score` lands at 914/916 rather than 916, and the `pipeline_health`
coverage gate exempts these no-code repos from the two source-dependent
dimensions.
...
| · HHI | `hhi_commits_git_5y` | 19 | 3,070 | 5,556 | 9,086 | 10,000 |
| **Complexity** | `score` | **1** | **26** | **50** | **73** | **100** |
| · lines of code | `loc_eoy` | 3 | 351 | 2,459 | 20,656 | 36,990,782 |
| · cyclomatic max | `cyclomatic_max` | 0 | 7 | 18 | 46 | 12,556 |
...
| · LOC / contributor | `loc_per_ac` | 2 | 105 | 296 | 789 | 353,301 |
...
| Step | Repos | % |
|---|---:|---:|
| input top repos | 916 | 100% |
| bus factor / HHI (git 5y) computed | 916 | 100% |
| bus factor / HHI (GitHub) computed | 914 | 99.8% |
| **Concentration score** | **916** | **100%** |
...
|---|---:|---:|
| input top repos | 916 | 100% |
| lines of code (scc) | 914 | 99.8% |
| cyclomatic max (lizard) | 914 | 99.8% |
| cognitive max (lizard) | 910 | 99.3% |
| churn 5y | 890 | 97.2% |
| **Complexity score** | **914** | **99.8%** |
...
|---|---:|---:|
| input top repos | 916 | 100% |
| OpenSSF score present | 916 | 100% |
| semgrep SAST present | 891 | 97.3% |
| CVE count 5y > 0 | 199 | 21.7% |
| OSS-Fuzz enrolled | 134 | 14.6% |
| CII Best Practices badge | 31 | 3.4% |
| **Security score** | **916** | **100%** |
...
|---|---:|---:|
| input top repos | 916 | 100% |
| issues data present | 916 | 100% |
| per-AC ratios (loc/cve/nni) computed | 914 | 99.8% |
| `issue_close_ratio` computed | 837 | 91.4% |
| `issue_trend_score` computed | 633 | 69.1% |
| **Workload score** | **914** | **99.8%** |
...
This is the same 916-repo scope the risk stage now runs on (both stages include
archived); the four checks per repo roll into
...
| Category | Repos | % |
|---|---:|---:|
| intent — any funding signal | 720 | 78.6% |
| intent — no funding signal | 196 | 21.4% |
