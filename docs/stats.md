...
**Risk dimensions cover the 920 top repos** — the valid class-A set
(`risk_input.value_classes = ["A"]`), non-archived; each funnel below starts
from those 920. The top-repo set now spans both platforms in
`settings.json top_repos.platforms` (`["github", "gitlab"]`): 26 GitLab repos
were added and are risk-scored exactly the same way, with percentiles ranking
the combined GitHub+GitLab population. `risk.csv` ranks all 920 on the four
scored dimensions; the funding signals and the intent/nonprofit flags moved to
the [Eligibility](#eligibility) stage. Methodology: [risk.md](risk.md) + component docs.
...
| Step | Repos | % |
|---|---:|---:|
| input top repos | 920 | 100% |
| bus factor / HHI (git 5y) computed | 920 | 100% |
| bus factor / HHI (GitHub) computed | 877 | 95.3% |
| **Concentration score** | **920** | **100%** |

73.5% of top repos have a git `_5y` bus factor of 1 (a single author covers ≥50%
of 5-year commits).
...
|---|---:|---:|
| input top repos | 920 | 100% |
| lines of code (scc) | 920 | 100% |
| cyclomatic max (lizard) | 920 | 100% |
| cognitive max (lizard) | 919 | 99.9% |
| churn 5y | 860 | 93.5% |
| **Complexity score** | **920** | **100%** |
...
|---|---:|---:|
| input top repos | 920 | 100% |
| OpenSSF score present | 894 | 97.2% |
| CVE count 5y > 0 | 205 | 22.3% |
| OSS-Fuzz enrolled | 131 | 14.2% |
| CII Best Practices badge | 30 | 3.3% |
| **Security score** | **920** | **100%** |
...
|---|---:|---:|
| input top repos | 920 | 100% |
| issues data present | 893 | 97.1% |
| per-AC ratios (loc/cve/nni) computed | 920 | 100% |
| `issue_close_ratio` computed | 821 | 89.2% |
| `issue_trend_score` computed | 622 | 67.6% |
| **Workload score** | **920** | **100%** |
...
archived repos (920 risk scope + 22 archived, which surface below as
...
AND active`. The 26 GitLab repos in scope are scored but currently **ineligible**
(`intent=false`, `eligible=false`): funding/intent detection is GitHub-coupled
and GitLab funding attribution is deferred, so no GitLab repo yet carries a
funding signal. Methodology: [eligibility.md](eligibility.md) +
...
The known non-OSS are content-licensed data repos (CC0/CC-BY — free for
documents, not software OSS by this model's strict policy) plus a handful of
newly-scoped GitLab repos; the unknown rows are GitLab repos whose license is
not yet resolved from a registry.
...
| Category | Repos | % |
|---|---:|---:|
| intent — any funding signal | 719 | 76.3% |
...
soliciting support — a higher-priority target for outreach; the 26 newly-scoped
GitLab repos all fall here (GitLab funding attribution is deferred).
