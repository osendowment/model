...
| `score` | 921 | ≥ 2 components present; 2 GitLab class-A repos scored — only those with an explicit `eco_crit`; the rest have just `top_eco_pct` and stay blank |

`score` range 0–100: **min 17.9 / mean 53.7 / max 81.4**.
...
| **Security** | `score` | **50** | **50** | **72** | **83** | **100** |
...
|---|---:|---:|
| input top repos | 894 | 100% |
| issues data present | 893 | 99.9% |
| per-AC ratios (loc/cve/nni) computed | 894 | 100% |
| `issue_close_ratio` computed | 821 | 91.8% |
| `issue_trend_score` computed | 622 | 69.6% |
...
| Category | Repos | % |
|---|---:|---:|
| intent — any funding signal | 719 | 78.5% |
| intent — no funding signal | 197 | 21.5% |
