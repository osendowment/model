...
- [End-to-end funnel](#end-to-end-funnel) — the whole pipeline in one table
...
The whole pipeline in one table: package universe → distinct repos → valid →
class A → the top-repo scope, then the parallel filters over that scope
(value/risk scoring, eligibility) down to the priority-ranked rows of
`data/preview/repos.csv`. The pre-scope stages narrow against the previous
row; the post-scope stages are each expressed against `top repos`.
...
|---|--:|---|
| `pr_score` | 12,076 | every group with any PageRank signal (all 12,082 rows minus 6 rank-less orphans); computed by `unify_value_data`; a `value_score` component |
...
| · net-new-issues / contributor | `nni_per_ac` | -9.93 | 0.00 | 0.29 | 1.00 | 315.00 |
...
|---|---:|---:|
| input top repos | 940 | 100% |
| issues data present | 940 | 100% |
| per-AC ratios (loc/cve/nni) computed | 940 | 100% |
| `issue_close_ratio` computed | 847 | 90.1% |
| `issue_trend_score` computed | 642 | 68.3% |
