...
**Scope**: every valid class-A **GitHub** repo, **archived included** (`load_github_top_slugs()`, which includes archived by default). The host filter matters: the tool scores a GitHub URL, so a GitLab slug like `gnome/glib` either resolves to an unrelated GitHub repo of the same name or fails outright. It once used the unfiltered loader, and the 17 GitLab repos that failed were retried on every run — an error row never caches as fresh — costing ~45s a run. `apply_criticality` joins only `platform == github` rows regardless, so `value.csv` `openssf_crit` is blank for every GitLab repo either way. TTL 365 days; error rows always retry. An idempotent healing pass runs per invocation: it backfills `repo_id`s that were unresolvable at fetch time and drops rows superseded by a repo rename (old-slug row vs new-slug row for the same id).
