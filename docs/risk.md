...
continuous scores and percentiles end-to-end — there are **no A–D risk tiers**.
...
`AC` = `active_contributors_git_5y`, distinct non-bot contributors who authored
a commit in 2021–2025 (git-clone method). Each ratio is percentile-ranked
(Hazen, 0–100) into `loc_per_ac_p` / `cve_per_ac_p` / `nni_per_ac_p`; the
dimension `score` is their geometric mean, empty when an input is missing or
AC = 0.
